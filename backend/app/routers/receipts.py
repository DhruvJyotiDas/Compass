...
from app.models import Communication, CommunicationEvent, EVENT_RANK
...
async def ingest_receipt(
    ...
    # Attribution: if clicked, check for orders within 72h
    if payload.event_type == "clicked":
        asyncio.create_task(_attribute_orders(str(comm.id), str(comm.customer_id), db))
...
async def _attribute_orders(comm_id: str, customer_id: str, db: AsyncSession):
    """Mark orders placed within 72h after click as attributed to this communication."""
    try:
        await db.execute(
            text("""
            ...
            """),
            {"comm_id": comm_id, "cid": customer_id},
        )
        await db.commit()
    except Exception:
        pass
